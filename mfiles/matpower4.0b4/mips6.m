...
%   MIPS
%   $Id: mips6.m,v 1.16 2010/05/13 15:42:39 ray Exp $
...
df = df * opt.cost_mult;
if nonlinear
    [hn, gn, dhn, dgn] = feval(gh_fcn, x, varargin{:}); %% non-linear constraints
...
%     dxdlam = AAA \ bbb;
    ...
    if opt.step_control
        ...
        if nonlinear
            [hn1, gn1, dhn1, dgn1] = feval(gh_fcn, x1, varargin{:}); %% non-linear constraints
    ...
    if sc
        ...
        for j = 1:opt.max_red
            ...
            f1 = f1 * opt.cost_mult;
            if nonlinear
                [hn1, gn1] = feval(gh_fcn, x1, varargin{:});    %% non-linear constraints
                h1 = [hn1; Ai * x1 - bi];   %% inequality constraints
                g1 = [gn1; Ae * x1 - be];   %% equality constraints
            else
                h1 = Ai * x1 - bi;          %% inequality constraints
